...
class MainWindow(base, form):
    def __init__(self):
        ...
        self.authorEdit.setText(os.getlogin())
    ...
    def closeEvent(self, event):
        #Resubmits everything to database on exit if sModel exists
        ...
        if self.sModel:
            ...
            if action == viewcp:
                ...
                if cp.endswith('.prc'):
                    ...
                    self.cmdLabel('Viewing CP: ' + cp)
                    viewer = CpViewer(os.path.join(self.cpDir, cp), self)
                    viewer.show()
                else:
                    logging.info(' No CP to View')
                    self.cmdLabel.setText('No CP to View')
                
            #Handles step execution
            if action == stepx:
                index = self.sModel.index(self.tableView.rowAt(pos.y()), 2)
                self.sModel.setData(index, True, self.sModel.ExecuteRole)
                self.sModel.dataChanged.emit(self.sModel.index(_row, 0),
                                              self.sModel.index(_row, self.sModel.columnCount()-1),
                                              [QtCore.Qt.BackgroundRole])
                logging.info(' Executed Step: ' + str(_row + 1))
                self.cmdLabel.setText('Executed Step: ' + str(_row + 1))
            if action == stepdx:
                index = self.sModel.index(self.tableView.rowAt(pos.y()), 2)
                self.sModel.setData(index, False, self.sModel.ExecuteRole)
                self.sModel.dataChanged.emit(self.sModel.index(_row, 0),
                                              self.sModel.index(_row, self.sModel.columnCount()-1),
                                              [QtCore.Qt.BackgroundRole])
    ...
    def getPData(self, index):
        ...
        #Also changes FD command buttons
    ...
    def pushPacr(self):
        ...
        _row = self.pTable.currentIndex().row()
        if self.pModel.data(self.pModel.index(_row, 7), role = self.DR) \
            == 'Dev':
            self.savePacr()
    ...
    def checkReview(self):
        #Checks whether there is a PACR waiting to be reviewed and sets the PACR review counter
        pActions = 0
        if self.pModel:
    ...
    def pacr2step(self, pRow):
        #Preforms PACR operations on main script table
        pChange = self.pModel.data(self.pModel.index(pRow, 3), role = self.DR)
        sRow = int(self.pModel.data(self.pModel.index(pRow, 2), role = self.DR)) - 1
        if self.sModel.editScript(pChange, self.pModel, pRow, sRow):
            logging.info(' ' + pChange + ' PACR ID: ' + str(pRow + 1))
            self.cmdLabel.setText(pChange + ' PACR ID: ' + str(pRow + 1))

            self.tableView.resizeColumnToContents(2)
            self.tableView.resizeColumnToContents(3)
            self.tableView.resizeRowsToContents()

        else: self.warn('Unknown')
